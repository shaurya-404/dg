...
function chkcoli(fx, fy) {
    ...
    for (let i = 0; i < map.length; i++) {
        ...
        if (e === 0) {
            if (chkrect(fx, fy, x, y, dpx - x, wt)) return true; // Wall left of door
            if (chkrect(fx, fy, dpx + dw, y, (x + squaresize) - (dpx + dw), wt)) return true; // Wall right of door
        } else {
            if (chkrect(fx, fy, x, y, squaresize, wt)) return true; // Solid top wall
        }
...
function playermove(){
    ...
    ctx.strokeStyle = 'black';
    ctx.stroke();
    healthbar(px, py, phealth, pmaxhealth);
...
function gameLoop() {
    ...
    // =====================================
        ...
        for (i = 0; i < map.length; i++) {
            ...
            if (enemyHealth <= 0) continue; // Dead enemies don't shoot!

            // Check if player is inside this exact room
            ...
            // If player is in the same room AND within detection range
            if (playerInRoom && distToPlayer < squaresize * 1.5) {
                // If 1000 milliseconds (1 second) has passed since last shot
                // ... inside the Enemy AI detection block ...
                if (currentTime - lastShot > 2500) {
                    const angle = Math.atan2(py - enemyY, px - enemyX);
                    const bSpeed = 0.5; // Enemy bullets are slightly slower
                    
                    // NEW: Push the enemy bullet as an Array [x, y, vx, vy, isPlayer, bounces]
                    bullets.push([
                        enemyX + Math.cos(angle) * radius, // b[0]: x
                        enemyY + Math.sin(angle) * radius, // b[1]: y
                        Math.cos(angle) * bSpeed,          // b[2]: vx
                        Math.sin(angle) * bSpeed,          // b[3]: vy
                        false,                             // b[4]: isPlayer (false = enemy)
                        0                                  // b[5]: bounces
                    ]);
                    
                    map[i][8] = currentTime; // Reset their cooldown timer
                }
            }
        }
        ...
        // =====================================
        // We loop backwards so we can safely delete bullets that hit things
        for (i = bullets.length - 1; i >= 0; i--) {
            let b = bullets[i];
            let hit = false;

            // X-Axis Move and Bounce
            b[0] += b[2];
            if (chkcoli(b[0], b[1])) {
                b[0] -= b[2]; 
                b[2] *= -1;  
            ...
            // NEW: Delete bullet if it has bounced 5 or more times
            if (b[5] >= 5) {
                bullets.splice(i, 1);
                continue;
            }

            // Delete bullet if it flies off the canvas
            ...
            if (b[4]) {
                ...
                for (j = 0; j < map.length; j++) {
                    if (map[j][7] <= 0) continue; // Skip dead enemies
                    
                    let dist = Math.hypot(b[0] - map[j][2], b[1] - map[j][3]);
                    if (dist < radius + 5) { // Radius + bullet size
                        map[j][7] -= 5; // Deal 5 Damage!
                        hit = true;
                        break;
                    }
                }
                ...
                let dist = Math.hypot(b[0] - px, b[1] - py);
                if (dist < radius + 5) {
                    phealth -= 5; // Deal 5 Damage!
                    if (healthText) healthText.innerText = phealth; // Update UI
                    hit = true;
                }
            }

            // Remove the bullet if it successfully hit someone
            if (hit) {
                bullets.splice(i, 1);
            }
        }
